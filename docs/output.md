...
```bash
cwctl labels list                 # table (default; colored on a TTY, NO_COLOR honored)
cwctl labels list -o json         # full fidelity — unknown API fields are never dropped
cwctl labels list -o yaml
cwctl labels list -o csv          # cells sanitized against =SUM() formula injection
cwctl labels list -o id           # one id per line, xargs-ready
```
...
```bash
cwctl agents list --columns name,email          # exact columns, in that order
cwctl conversations list --jq '.[0].meta.sender.name'   # gojq over the response
```

## Filtering & pagination

```bash
cwctl conversations list --status open --labels vip     # server-side params
cwctl agents list --filter role=agent                   # client-side field=value
cwctl contacts list --page 3                            # one server page
cwctl contacts list --all                               # walk every page
cwctl contacts list --all --limit 50                    # stop the output at 50
```
...
```bash
$ cwctl webhooks create --url https://example.com/hook --dry-run
